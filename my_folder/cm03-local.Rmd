...
2.8 because two close points are 2.9 and near 2.7
...
```{r}
dat.lowess = filter(dat.a,d<1)
y.preduct2 = mean(dat.lowess$y)

```

3. What happens when you try to pick an $r$ that is way too small? Say, $r=0.01$? Why?
If r is too small, it is possible we can't find any value. May have the problem of over fitting.
...
If r is too large, there will be smaller variance but a large bias.
...
dat$d <- abs(dat$x-x)
dat.a1 = arrange(dat,d)
dat.knn1 = dat.a1[1:5, ]
y.hat = mean(dat.knn1$y)
return(y.hat)    
...
dat$d <- abs(dat$x-x)
dat.a = arrange(dat,d)
dat.lowess1 = filter(dat.a,d<1)
y.hat = mean(dat.lowess1$y)
return(y.hat)      
...
Manhattan distance -> The ordered data (Months)
Distance between categorical variable.
## Scaling
...
Standerlize the variable and use the Z score.
